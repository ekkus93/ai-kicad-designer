"""Create the one-time M2h1 implementation/environment freeze before V6 authoring."""
...
def main() -> None:
    forbidden = [HERE / "corpus.v6.json", HERE / "ir"]
    if any(path.exists() for path in forbidden):
        raise SystemExit("refusing to freeze after V6 corpus/IR authoring")

    status = subprocess.check_output(
        ["git", "status", "--porcelain"], cwd=ROOT, text=True
    ).splitlines()
    allowed = {"?? fixtures/m2blind_v6/"}
    if set(status) - allowed:
        raise SystemExit(f"working tree was not clean before freeze tooling: {status}")
    ...
    freeze = {
        "schema_version": "m2h1.implementation-freeze.v1",
        "evaluation": "M2h1 — corpus.v6 frozen blind qualification",
        "hash_algorithm": "sha256",
        "git_head": subprocess.check_output(
            ["git", "rev-parse", "HEAD"], cwd=ROOT, text=True
        ).strip(),
        "git_status_porcelain_before_freeze": [],
        "python": {
        ...
        "locks": {
            name: {"path": path, "sha256": sha256(ROOT / path)} for name, path in locks.items()
        },
        ...
        "historical_evidence_before": historical,
    }
