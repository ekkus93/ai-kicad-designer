...
Connectivity-changing or illegal mutation cases must all be rejected: remove one branch, move a wire off a pin by one grid unit, add a junction to a legal unrelated-net crossing (the unchanged interior crossing without a junction must remain a passing control), merge a rail with a reference, duplicate a terminal, omit a power unit, mis-map a package pad, rename a required net, confuse identical local names across sheets, attach a bus member incorrectly, connect an NC pin, swap amplifier inputs, or short a net outside an approved tie footprint. Include invisible/hard-to-see defects. The NE5532 current baseline is a permanent rejection fixture even if its visual score rises.
...
Starting in M2, use two reviewers with relevant electronics experience for quality milestone gates, including one who did not author the layout rule. Ask them to identify blocks, trace a named input-to-output path, explain feedback/timing behavior, locate supply/return and decoupling, and map an interface across sheets. Record correctness, time and confusion points, plus 1–5 ratings for organization, path traceability, motif recognition, labeling and drafting consistency.

Initial acceptance: both reviewers report no severe defect, every dimension has a median of at least 4/5, no individual rating is below 3, and trace tasks are answered correctly. Compare trace time to a reviewed human reference where one exists; a >25% slowdown triggers investigation rather than automatic universal rejection. A quality milestone from M2 onward needs at least 90% acceptance on all valid held-out requests inside its preregistered capability envelope (including generation failures, timeouts and quality rejections in the denominator) and no known severe regression. A failed design remains visible in the denominator and limits the qualified envelope. Round the required accepted count upward: 90% of eight cases requires eight accepted cases.
...
For each milestone record corpus revision, toolchain/policy hashes, all gate results, per-family acceptance, worst cases, runtime/memory distributions, deterministic reruns and outstanding unsupported capabilities. M1 needs deterministic electrical vertical slices, not a general readability claim. M2/M3 need real analog/hierarchy quality evidence. M4 needs constrained placement and router-boundary evidence. M5 needs manufacturing exports and a physical demonstrator. M6 needs breadth and held-out review; M7 then proves conversational authoring before product infrastructure is considered. Exact entry/exit requirements live in [the implementation plan](IMPLEMENTATION_PLAN.md).
...
## 8. Review corrections: independent authority and practical gates

M1 uses one divider plus an intentional-NC connector variant, one engineering reviewer and three repeated clean builds, as specified in the implementation plan. It does not claim motif generalization, full IR coverage, multi-unit support, a frozen corpus score or image-byte reproducibility. From M2, solver-using accepted cases have five reruns and the blinded rubric; router five-run qualification starts at M4. Raw tool-owned SVG/container ordering may differ only under the enumerated nonsemantic normalization contract; geometry, text, pin identity, connectivity and violations remain exact.

Separate identity mistakes from connectivity mistakes. Mutate an emitted pin number or symbol definition while leaving intended pin maps/metadata unchanged; the observer must reject it. Remove an NC marker while leaving its expected terminal absent from XML; inventory and actual NC checks must reject it. Copy a child sheet under another instance with the same local net names; observation must distinguish instances. Never populate the observed graph by iterating expected net members and merely confirming that labels exist.

A motif detector and the acceptance evaluator must not certify each other solely by reusing the same chosen match. Recompute structural facts from actual artifacts/resolved assets, use independently authored semantic annotations, and retain blind trace tasks. Qualification includes diagrams with legal spacing but reversed stage order, disconnected-looking feedback islands, misleading local/global names, bus member disorder and detached decoupling. Electrical correctness alone will not catch these readability failures.

Keep hard legality separate from family readability preferences: co-located asset-defined pin stacks and intended common conductor geometry are not unrelated collisions. Invalid requests go to a separate validation denominator with rejection evidence; valid supported requests that fail generation stay in quality/completion denominators. Freeze capability exclusions before testing and publish per-family completion plus acceptance; an optimizer cannot improve its headline score by declining difficult valid cases.

Operating-analysis obligations may be discharged by reviewed external calculations/simulation/measurement bound to the exact design, assumptions and artifact hashes. This avoids requiring a general electrical/thermal simulator in early milestones. Accepted input assertions and ERC do not discharge those obligations. The fabricated demonstrator remains an M5 evidence goal, not a requirement for declaring M1–M4's narrower results successful.
