...
These are eventual ownership boundaries and typed function interfaces, not dynamically loaded plugins. Use Python 3.12, strict Pydantic 2 models at serialization boundaries, frozen internal records and pytest. Emit JSON Schema 2020-12 for the implemented capability profile from the boundary models and test its agreement with this specification. Use Ruff and static type checking. Pin dependencies in a lockfile at implementation time.

OR-Tools CP-SAT is a bounded feasibility/legalization option beginning in M2, not a mandatory optimizer at every placement layer. Start with deterministic constructive placement and local repair; add a solver only where a measured conflict justifies it. Its integer model fits finite orientation choices and separation constraints ([solver documentation](https://developers.google.com/optimization/cp/cp_solver)). Use custom semantic decomposition and deterministic orthogonal routing, not a monolithic solver over every wire coordinate. Native acceleration is postponed until profiling identifies a concrete hotspot.
...
- `ArtifactMap`: a bijection between instantiated components and physical reference identities, plus explicit one-to-many terminal-to-symbol-occurrence and terminal-to-pad-occurrence relations. Keys include sheet instance, symbol use/unit/style and asset occurrence; pad occurrences include footprint and primitive identity. These relations identify observations, never supply observed pin membership. The observer cross-checks actual embedded assets, pin numbers and instances against the independently resolved inventory before using the map. See the IR identity contract.
...
The reproducibility unit is identical Circuit IR, resolved assets, policy, compiler and toolchain on the recorded Linux execution environment/architecture. M1 qualifies only that local environment; portable image qualification is added before solver/router milestone claims. Do not promise identical optimization across different solver releases or CPUs without a cross-platform qualification.

Use integer nanometers for both physical and schematic geometry (grid pitch is a policy, not the storage unit); use decimal electrical quantities. Sort unordered collections by stable ID, preserve intentional order, reject duplicate JSON keys and nonfinite numbers. UUIDv5 names derive from `design.id / artifact-kind / stable-object-id`; never use random UUIDs or wall time. The build key hashes all inputs, while stable object UUIDs do not depend on unrelated objects' content.
...
Compiler-owned canonical KiCad documents, normalized geometry and normalized semantic reports must reproduce exactly. Tool-owned container ordering and SVG bytes are not the correctness contract; compare parsed vector geometry/text/style and electrical results, allowing only explicitly enumerated nonsemantic differences. Raster antialiasing and optional 3D images are review aids, not byte-identity gates. Keep raw exports with hashes, including tool timestamps where present; normalize only an enumerated set of nonsemantic metadata for comparisons. Copper, coordinates, net names, text and violations are never normalized away. Human/visual review is a separate signed observation over artifact hashes, not part of the pure compiler.
...
## 9. Reviewed implementation boundary

The [adversarial review](ARCHITECTURE_REVIEW.md) narrows M1 to one divider and its mutation/NC variants. Module names above are ownership boundaries, not a requirement to scaffold every package, persist every intermediate, or implement the full future schema in M1. The milestone's explicit capability profile governs implementation scope. Unimplemented future payloads are rejected, never accepted as opaque operational constraints.

Constraint obligations have an explicit verification stage: input, schematic, placement, routed board, manufacturing or operating analysis. A schematic target rejects unsupported schematic obligations, while reporting later physical/operating obligations as unevaluated. Release requires all mandatory obligations; an externally reviewed engineering analysis bound to input/artifact hashes can discharge an operating requirement without building a general simulator. A user requirement assertion alone cannot do so.

Future conversational integration remains a separate authoring client. It consumes a versioned schema/capability description, accepted IR digest, diagnostics and evidence references; it returns `RevisionProposal = {base_digest, schema_version, patch, rationale, evidence_refs, unresolved_questions}`. Use RFC 6902 operations against the exact base digest, with no concurrent blind merge. For initial creation, `base_digest` is null and the patch adds the entire document at the root; later proposals require the exact accepted digest. Apply to an isolated candidate, validate all references and supported capabilities, present electrical/part/constraint changes for explicit acceptance, then compile the accepted snapshot. Stale proposals fail; rejected builds never mutate the accepted design. Tool use for part research/custom-asset authoring stays outside deterministic compilation. Model version, prompts and conversation provenance are authoring records, not reproducibility dependencies once the accepted IR and assets are fixed. M7 makes this boundary executable without moving it into M1.
