...
For KiCad power semantics, keep an explicit adapter test matrix covering visible power symbols, hidden power inputs, local references and split supplies. KiCad 10 distinguishes global and local power-symbol definitions; their value identifies the net in the applicable scope. Local labels are sheet-scoped, hierarchical labels connect through sheet pins, and hidden power-input pins can create global connections by pin name. A ground-shaped glyph alone establishes none of these semantics. [Schematic semantics](https://docs.kicad.org/10.0/en/eeschema/eeschema.html) The compiler's stricter policy is to reject any unresolved implicit connection and never normalize ground names by spelling.
...
Read real KiCad XML exports into a separate `ObservedElectricalGraph`, not back into the proposed IR with repair defaults. Resolve actual references and sheet paths through the checked component-instance map and explicit pin-occurrence relations; cross-check observed symbol identities, embedded definitions, physical pin numbers and selected unit/style. Do not remap observed pin numbers through an unchecked generator map. Unknown/duplicate physical references or missing required units fail; several units sharing one package reference are expected, not duplicate components. Compare the entire terminal inventory and net partition, not just selected named nets. XML may omit isolated pins or collapse graphical occurrences; therefore separately read actual schematic instances and embedded definitions for inventory/NC evidence. Never backfill absent observations from the intended net membership. Share a syntax tokenizer if useful, but do not reuse the emitter’s in-memory scene or connectivity decisions as the observer.
...
## 10. Lock and report contracts and narrow M1 profile

The table defines the eventual lock contracts. M1 implements the subset below rather than full toolchain-image management. These are proposed wire contracts, with `additionalProperties: false` and required fields unless marked optional. Asset and policy file references are paths relative to the lock file's parent; reject traversal outside the explicitly supplied bundle. Executable paths are absolute paths in the recorded M1 environment or later qualified execution image. SHA256 values are lowercase 64-character hex strings. Locks are created in an explicit preparation step and validated before compilation; the build never updates them.
...
`drafting.v1`'s M1 profile contains `grid_nm=1270000`, `text_height_nm=1270000`, `wire_gap_nm=1270000`, `symbol_gap_nm=2540000`, `block_gap_nm=5080000`, `margins_nm={left:20320000,top:25400000,right:20320000,bottom:25400000}`, `seed=0`, and a `power_assets` map, populated with reference/positive/negative/flag assets only when that capability is implemented. M1 has an empty `power_assets` map and uses explicit wires/local labels. Starting with M2, bind required entries to qualified definitions such as `power:GND`, `power:VCC`, `power:VEE` and `power:PWR_FLAG`; customize displayed net values only under the qualified power-symbol rule. Policy-inserted objects receive their own ArtifactMap entries. A flag for a return requires evidence that the external supply actually includes that return terminal; net spelling alone is insufficient.

M2 extends the profile with the bounded-search fields in the schematic specification, versioning the profile digest. A profile file is data under a known schema, not executable hooks. Initial M1 graphs/layouts may stay in memory; optional diagnostic serialization uses canonical typed JSON. Public persistence of every intermediate is not an M1 requirement. Layout objects have `id` and `origin_ids`; geometric fields use exact integers. Symbol uses reference `(component, asset, unit, body_style)`, wires reference a net ID and ordered endpoints, labels reference a net ID and scope, and text carries an occupied envelope. Logical terminal/net graphs have identity and membership without presentation coordinates. Object collections sort by ID; deliberate path/vertex orders remain ordered. Detailed public editing APIs are unnecessary for this first compiler slice.

### M1 implementation subset

Use lock `schema_version: "m1.1"` for the reduced profile; reject mixing it with the later `1.0` contract. Asset entries retain `id, kind, library_id, path, file_sha256, dependencies, source, license`; resolved digests are computed and recorded in the manifest rather than requiring a second asset preparation tool. Source/dependency/license records remain mandatory. Only the selected resistor and connector definitions and their actual dependencies need resolution.

The M1 tool lock has `schema_version, platform, executables, python_lock_sha256, configuration_digest, environment`; `platform` records OS, architecture and CPU identity, and environment records locale/timezone. No image or font-bundle digest is required when using KiCad's built-in stroke font and a recorded local environment. External fonts are unsupported in M1. Executable entries retain the table's fields. M1 policy lock and manifest retain the table's structures with `schema_version: "m1.1"`; only drafting and executed schematic checks are present. Check absence is not success. Raw execution times/logs stay outside canonical comparisons. Add image/font/capability locks only as their qualified features appear; translate explicitly to later lock versions.

M1 uses the built-in stroke font, fixed page/options and compares parsed SVG paths/text/style with stable numeric geometry. Harmless XML attribute ordering or generated element identifiers can be canonicalized only after proving they do not affect references/rendering. Keep original SVGs. Do not demand raw PNG, 3D-render or XML container bytes match when their interpreted content is unchanged. Missing geometry/text or differing electrical observations always fail. The same distinction applies to tool-owned board/report ordering; compiler-owned generated files remain byte reproducible.

### Qualification debt that cannot be inferred from documentation

- Verify NC/isolated-pin inventory and actual pin-number mutations in M1, before trusting the observer. M2 adds hidden power, common-unit pins, alternate styles, split rails and stacked distinct pins; M3 adds full repeated-instance paths and label scope collisions.
- Pin KiCad 10.0.6 as a defensible qualification target, not “latest forever.” Before M4, reconfirm bridge availability and stable headless alternatives. Do not invest in broad SWIG abstractions; only DSN/SES loading/export/import/save live there. Its removal risk does not affect the M1 schematic CLI path.
- No documentation proves DSN preserves project rules, zones, pad-stack details or protected copper. The [PCB qualification matrix](PCB_PLACEMENT_SPEC.md#9-router-qualification-matrix-and-physical-measurement-limits) is mandatory before a router-backed placement claim. Export/import-only tests and repeated real routing are separate tests.
