...
The stage sequence is resolve geometry → verify/recognize semantics → form blocks and motifs → choose sheets/interfaces → enumerate motif variants → coarse placement → fine placement/text → routing → independent checks → bounded repair → export and review. Persist the input/output of every stage with rule IDs and reasons.
...
A motif match is `{rule_id, rule_version, terminals, members, relationships, predicates, variants}`. Predicates are independently recomputable. Multiple possible matches are ranked by accepted relationship evidence, specificity, matched terminal coverage, then stable ID. Select a nonconflicting cover using bounded set-packing: a graphical symbol unit has one placement owner; a component's functions can belong to different motifs. Shared supplies are ports, not duplicated symbols. Supporting relationships may overlay that cover.
...
For each block, generate up to eight feasible motif compositions, keeping different aspect ratios and interface arrangements. CP-SAT variables select variant, orientation and integer x/y origins. Hard constraints enforce occupied-envelope separation, sheet bounds, authored orders, legal transforms and indivisible motifs. Soft tiers control semantic ordering, port alignment, spacing and estimated route demand. Sequentially optimize tiers while fixing achieved bounds; report whether each bound is proven or merely feasible. Integer objective scaling and maximum sums are checked for overflow.
...
- Use global labels for explicitly global interfaces only. Use conventional power symbols only for a genuinely global rail/reference with an exact approved value. A local reference or isolated return uses explicit wiring/hierarchical scope instead of a global ground symbol.
...
M1 implements only the divider/LED grammars and ordinary orthogonal connections on one sheet. Place a divider as two vertical resistors with a rightward tap and a separate connector region; LED/resistor as a simple series chain with preserved polarity. Derive spacing from occupied envelopes using the profile clearances, translate the composition to a grid-aligned origin within 20.32 mm left and 25.4 mm top margins, and route with exact pin coordinates. Enumerate finite legal transforms; no solver is needed for these two grammars. M2 introduces general motif composition and CP-SAT behind the same `SchematicLayout` interface. This narrow first slice proves the full electrical/export boundary before attempting general layout quality.
