...
4. Begin with deterministic candidate-pose enumeration and greedy placement/local repair for the tiny-board profile. Use CP-SAT to legalize a conflicting window or choose cluster orientations/regions only when constructive methods fail or comparative evidence justifies it. Integer search positions initially use a 0.1 mm lattice, refined to 0.025 mm near constrained pads; exact externally fixed coordinates remain exact.
...
## 9. Router qualification matrix and physical measurement limits

Run the following on small hand-placed boards before developing a general placer. A round-trip with no routing isolates bridge loss; a second run with routing tests actual rule enforcement. Each row needs positive and deliberately violating fixtures, source-board/DSN/SES/imported-board hashes, explicit projected settings and independent measurements. Documentation and a router's completion count are not sufficient.

| Contract | Experimental proof required | Failure policy |
|---|---|---|
| Coordinate and identity | Asymmetrical top/bottom footprints, nonzero origin, fixed connector/holes, alphanumeric/repeated pad numbers and multiple net classes round-trip with unchanged poses and terminal assignments | Block bridge qualification on any drift or mapping loss |
| Rules | Inspect DSN width/clearance/layer/via values; route narrow passages and verify actual minimum widths, clearances and drills in imported copper | Unsupported hard rules reject routing; do not rely on generic default classes |
| Obstacles/pad stacks | Concave outline, cutout, copper/track/via keepouts, NPTH hole, custom pad and through-hole obstruction survive export and affect routes | Limit supported shape/rule envelope explicitly |
| Fixed copper | Include locked track/via chains and already-routed nets; compare their geometry and verify remaining routing neither deletes nor shorts them | No protected-sensitive-route strategy until preservation is proven |
| Planes/zones | Determine whether zones are obstacles, conductive regions, omitted or transformed in DSN; after import refill the original zone model and check connectivity/clearance/return obligations | Zone-connected nets may not be declared complete from router airwire counts |
| Net ties/internal shorts | Separate distinct logical nets and localized allowed ties; repeated pad shapes remain separately inventoried | Reject any global merge or DRC waiver used as equivalence |
| Repeatability | Five clean processes/directories, same input/settings/JRE and fixed finite work budget; compare normalized tracks/vias, preserved objects and completion | Nondeterminism blocks automatic routed release, not placement-only work |
| Sensitive channels | Demonstrate coupled pair gap/skew, segment transitions, reference continuity and preserved critical loops before advertising them | Generic DSN width/clearance is not USB/SI/loop qualification |

Classify each obligation as native, conservatively projected, protected fixed geometry, or postchecked-only. A postchecked-only hard rule is acceptable only if an independent verifier can reject every violating candidate; bounded failure is expected and cannot be marketed as routing support. “Unsupported” applies when no trustworthy verifier/protection exists. Passing DRC does not verify impedance or thermal behavior.

Critical-loop path extraction needs a declared operating mode and participating terminal edges. Centerline paths in a unique track tree may support a conservative routed length/area metric; branches, zones, parallel paths, self-intersections and unknown component-internal trajectories make the metric ambiguous. Reject unsupported loop measurements or accept an explicit external analysis bound to the board/mode. Do not use cancelling signed areas or choose the shortest favorable branch. Switching-node copper extent includes filled zones, pads and tracks. High-current neckdowns/via arrays need current/thermal evidence separately from geometric loop compactness.

Externally fixed mechanics include tolerances and mating/access evidence. Validate actual transformed pad/body/access geometry, not solver rectangles. A shared multi-unit package remains one placement even when its schematic functions occupy different sheets. Physical clusters contribute overlapping constraints to that one package; they cannot each allocate a copy.
